              ...
              {items.map((item) => {
                ...
                return (
                  <li className="w-full flex items-center" key={item.title}>
                    <Link
                      href={item.url}
                      className={`${pathname === item.url ? "bg-main-color/60 text-white" : ""} 
                        w-full text-right px-5 py-4 rounded-sm font-bold flex items-center gap-3 hover:bg-main-color/60  hover:text-white duration-300`}>
